    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-ink-700/60">{supplier.name}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-forest-950/10 bg-white p-4">
          <p className="text-xs font-bold uppercase tracking-wide text-ink-700/50">Produtos vinculados</p>
          <p className="text-xl font-extrabold text-forest-950">{summary.count}</p>
        </div>
        <div className="rounded-2xl border border-forest-950/10 bg-white p-4">
          <p className="text-xs font-bold uppercase tracking-wide text-ink-700/50">Produtos ativos</p>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="flex flex-col gap-2 rounded-3xl border border-forest-950/10 bg-white p-6">
          <p className="mb-2 text-xs font-bold uppercase tracking-wide text-ink-700/50">Dados do fornecedor</p>
        ...
        <div className="flex flex-col gap-2 rounded-3xl border border-forest-950/10 bg-white p-6">
          <p className="mb-2 text-xs font-bold uppercase tracking-wide text-ink-700/50">Produtos vinculados</p>
          {supplierProducts.length === 0 ? (
            <p className="text-sm text-ink-700/60">Nenhum produto vinculado a este fornecedor ainda.</p>
          ) : (
              ...
              {supplierProducts.map((product) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-ink-700/60">{product.presentation} · {product.weight}</p>
                  </div>
